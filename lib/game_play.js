...
export function drawWindups(windups, context, dino) {
  windups.forEach(function(windup) {
    var otherWindup = returnOtherWindup(windups, windup);
    windup.move(dino, otherWindup).draw(context);
  });
}

function returnOtherWindup(windups, initialWindup){
  for (var i = 0; i < windups.length; i++ ) {
    if (initialWindup !== windups[i]) {
      return windups[i];
    }
  }
}
